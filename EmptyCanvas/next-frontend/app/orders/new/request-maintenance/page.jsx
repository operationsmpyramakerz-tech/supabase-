...
export default async function ShoppingCartPage({ searchParams }) {
  ...
  const flow = "request-maintenance";
  const initialType = "Request Maintenance";
  const title = "Request Maintenance";
  const editMode = String(query?.edit || "") === "1";
  const editKey = String(query?.editKey || "").trim();
  const publicPath = `/next/orders/new/${flow}`;
  const loginNext = `${publicPath}${editMode ? "?edit=1" : ""}${editKey ? `${editMode ? "&" : "?"}editKey=${encodeURIComponent(editKey)}` : ""}`;
  const bootstrapUrl = `/api/page-bootstrap?scope=shopping-cart&type=${encodeURIComponent(initialType)}`;
      ...
      <main className="standalone-state">
        <section className="state-card">
          ...
          <div className="actions">
            ...
            <a className="secondary-button" href={`/orders/new?type=${encodeURIComponent(initialType)}&classic=1`}>Open classic page</a>
          </div>
        </section>
      </main>
    );
    ...
    return (
      <main className="standalone-state">
        <section className="state-card">
          ...
          <div className="actions">
            <a className="primary-button" href={`/orders/new?type=${encodeURIComponent(initialType)}&classic=1`}>Open classic Shopping Cart</a>
  ...
  if (!account) redirect(`/login?next=${encodeURIComponent(publicPath)}`);
  ...
  const initialDraft = getResource(resources, "/api/order-draft", null);
  ...
  return (
    <AppShell
      ...
      classicHrefOverride={`/orders/new?type=${encodeURIComponent(initialType)}`}
