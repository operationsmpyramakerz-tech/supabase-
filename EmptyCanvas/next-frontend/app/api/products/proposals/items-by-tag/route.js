...
import { addProposalProductsByTag } from "../../../../../lib/proposal-kit-service";
...
export async function POST(request) {
  ...
  try {
    ...
    const detail = await addProposalProductsByTag(id, await requestBody(request), gate.account);
    return NextResponse.json({ ok: true, source: "supabase-next", ...detail }, { status: 201 });
  } catch (error) {
    return errorResponse(error, "Failed to add tag components to proposal.");
  }
}
