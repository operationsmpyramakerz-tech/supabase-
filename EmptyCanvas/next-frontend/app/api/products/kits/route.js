...
export async function GET(request){const gate=await kitReadGate();if(!gate.ok)return gateResponse(gate);try{const url=new URL(request.url);const fresh=url.searchParams.get("_fresh")==="1";return NextResponse.json({ok:true,source:"supabase-next",kits:await listKits(gate.account,{fresh})},{headers:{"Cache-Control":"no-store"}});}catch(error){return errorResponse(error,"Failed to load kits.");}}
