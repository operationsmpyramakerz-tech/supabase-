...
export async function GET(_request,{params}){const gate=await kitReadGate();if(!gate.ok)return gateResponse(gate);try{const {id}=await params;return NextResponse.json({ok:true,source:"supabase-next",...(await getKit(id,gate.account))},{headers:{"Cache-Control":"no-store"}});}catch(error){return errorResponse(error,"Failed to load kit.");}}
