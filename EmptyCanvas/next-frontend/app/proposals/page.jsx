...
import { listKits, listProposals } from "../../lib/proposal-kit-service";
...
export default async function ProposalsPage() {
  ...
  const [catalogResult, proposalsResult, kitsResult, membersResult] = await Promise.allSettled([
    getProductsCatalog(),
  ...
  return (
    <AppShell account={gate.account} title="Proposals" eyebrow="Reusable quotation workspace" activePath="/next/proposals" bodyClass="products-page proposals-page" classicStyles={["/css/products.css?v=products-manual-image-v1", "/css/proposals.css?v=b2b-addname-transparent-pdf-v1"]}>
      <ProposalsClient account={gate.account} initialCatalog={catalog} initialProposals={proposals} initialKits={kits} initialMembers={members} bootstrapWarnings={warnings} />
    </AppShell>
  );
}
