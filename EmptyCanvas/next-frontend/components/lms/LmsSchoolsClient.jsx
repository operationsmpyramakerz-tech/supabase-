...
export default function LmsSchoolsClient({ initialSchools, initialStocktakingColumns, initialCreate = false, initialEditId = "", access, bootstrapWarnings = [] }) {
  ...
  return (
    <main className="next-lms-schools-page">
      ...
      {bootstrapWarnings.length ? <div className="dashboard-notice" role="status"><strong>Some school support data could not load.</strong><span>The available school records are shown below. Use Refresh to retry.</span></div> : null}

      <section className="next-lms-schools-hero">
        <div><span className="next-lms-kicker">LMS school operations</span><h2>Schools workspace</h2><p>Manage school contracts, academic coverage, contacts, capacity, and Stocktaking links from one protected workspace.</p><div><a href="/next/lms">LMS Overview</a></div></div>
