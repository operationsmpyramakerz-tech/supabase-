...
export default function LmsUsersCenterClient({ initialStructures, initialSchools, initialRoles, initialTab = "structures", access, bootstrapWarnings = [] }) {
  ...
  return (
    <main className="next-lms-users-page">
      ...
      {bootstrapWarnings.length ? <div className="dashboard-notice" role="status"><strong>Some LMS directory data could not load.</strong><span>The available structures and roles are shown below.</span></div> : null}
      ...
      <section className="next-lms-users-hero">
        ...
        <div className="next-lms-users-hero-actions"><a href="/next/lms">LMS Overview</a>{canEdit ? <button type="button" onClick={() => setBuilderOpen(true)}>Add Structure</button> : null}</div>
      </section>
