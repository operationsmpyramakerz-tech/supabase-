...
function DetailItem({ label, value }) {
  return (
    <div className="next-events-detail-item">
      <small>{label}</small>
...
function DetailsModal({ event, canOpenRequests, onClose }) {
  ...
  return (
    <div className="next-modal-layer" role="presentation" onMouseDown={(mouseEvent) => { if (mouseEvent.target === mouseEvent.currentTarget) onClose(); }}>
      <section className="next-modal next-events-details-modal" role="dialog" aria-modal="true" aria-label="Scheduled event details">
        ...
        <footer className="next-events-modal-actions">
          <div>
            <a className="secondary-button" href={`/api/events/${encodeURIComponent(event.id)}/pdf`} target="_blank" rel="noreferrer">Download PDF</a>
            {canOpenRequests ? <a className="secondary-button" href="/next/events">Open Event Requests</a> : null}
          </div>
...
export default function EventsCalendarClient({ account, initialEvents = [], bootstrapWarnings = [] }) {
  ...
  return (
    <section className="next-events-calendar-page">
      <div className="next-events-calendar-title-row">
        <div>
          <span className="next-events-kicker">Live event schedule</span>
          <h2>Plan and monitor event execution</h2>
          <p>Select a date to review upcoming work, identify schedule conflicts, and open complete event details.</p>
        </div>
        <div className="next-events-title-actions">
          <button type="button" className="secondary-button" onClick={() => selectDate(new Date())}>Today</button>
          <button type="button" className="secondary-button" onClick={refresh} disabled={loading}>{loading ? "Refreshing…" : "Refresh"}</button>
          {canCreate ? <button type="button" className="primary-button" onClick={openNewEvent}>Add New Event</button> : null}
        </div>
      </div>
      ...
      <div className="next-events-calendar-summary">
        <article><small>Scheduled events</small><strong>{datedEvents.length}</strong><span>{events.length - datedEvents.length} awaiting dates</span></article>
        <article><small>This month</small><strong>{activeMonthEvents.length}</strong><span>{month.toLocaleDateString("en-GB", { month: "long", year: "numeric" })}</span></article>
        <article><small>Selected date</small><strong>{selectedDayEvents.length}</strong><span>{formatDate(selectedDate, { weekday: "short", day: "2-digit", month: "short" })}</span></article>
        <article><small>Upcoming</small><strong>{upcoming.length}</strong><span>From selected date</span></article>
      </div>

      <div className="next-events-calendar-layout">
        <section className="next-events-calendar-card">
          <header>
            <button type="button" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))} aria-label="Previous month">‹</button>
            <div><h3>{month.toLocaleDateString("en-GB", { month: "long" })}</h3><span>{month.getFullYear()}</span></div>
            <button type="button" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))} aria-label="Next month">›</button>
          </header>

          <div className="next-events-calendar-weekdays" aria-hidden="true">
            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => <span key={day}>{day}</span>)}
          </div>

          <div className="next-events-calendar-grid">
            {calendarDays.map((day) => {
              ...
              return (
                <button
                  type="button"
                  key={dateKey(day)}
                  className={`${outside ? "outside " : ""}${selected ? "selected " : ""}${current ? "today " : ""}${scheduled.length ? "has-events" : ""}`.trim()}
        ...
        <aside className="next-events-calendar-side">
          ...
          <section className="next-events-calendar-list-card">
            <div className="next-events-calendar-tabs">
              <button type="button" className={activeList === "upcoming" ? "active" : ""} onClick={() => setActiveList("upcoming")}><span>Upcoming</span><b>{upcoming.length}</b></button>
              <button type="button" className={activeList === "past" ? "active" : ""} onClick={() => setActiveList("past")}><span>Past</span><b>{past.length}</b></button>
            </div>
            <div className="next-events-calendar-list-head"><div><small>{activeList === "past" ? "Before today" : `From ${formatDate(selectedDate, { day: "2-digit", month: "short", year: "numeric" })}`}</small><h3>{activeList === "past" ? "Past Events" : "Upcoming Events"}</h3></div><b>{list.length}</b></div>
            <div className="next-events-calendar-event-list">
              {list.length ? list.map((event) => (
                <button type="button" key={event.id} onClick={() => setActiveEvent(event)}>
                  <span className="event-date"><strong>{formatDate(startDate(event), { day: "2-digit", month: "short" })}</strong><small>{startDate(event)?.getFullYear() || "—"}</small></span>
                  <span className="event-copy"><small>{event.organizationName || event.governorate || "Event execution"}</small><strong>{event.eventName || "Untitled Event"}</strong><em>{typeLabel(event)} · {event.governorate || "Location to be confirmed"}</em></span>
