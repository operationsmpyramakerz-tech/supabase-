...
const SUMMARY_BASE_SELECT = [
  ...
  "quantity_remaining",
  "status",
  // These maintenance-log columns are intentionally kept in the compact DB
  // projection because they affect the effective workflow status.
  "serial_number",
...
const SUMMARY_SEARCH_SELECT = [...SUMMARY_BASE_SELECT, "product_name"].join(",");
...
export async function loadCurrentOrdersInitialPage({ account, limit = PAGE_LIMIT } = {}) {
  return await measurePerformance("page-data", "orders.current.initial", async () =>
    await loadCurrentOrdersPage({ account, tab: "all", type: "all", query: "", cursor: null, limit }),
  );
}
