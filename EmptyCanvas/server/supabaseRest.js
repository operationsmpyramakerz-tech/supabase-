...
async function selectWhere(table, column, value, { limit = 5000, op = 'eq' } = {}) {
  const safeLimit = Math.max(1, Math.min(5000, Number(limit) || 5000));
  const safeColumn = encodeURIComponent(String(column || '').trim());
  const safeOp = String(op || 'eq').replace(/[^a-z]/gi, '') || 'eq';
  return await request(`/${encodeTableName(table)}?select=*&${safeColumn}=${safeOp}.${encodeFilterValue(value)}&limit=${safeLimit}`);
}
...
async function deleteIn(table, column, values = []) {
  ...
  const safeColumn = encodeURIComponent(String(column || '').trim());
  const inList = list.map((v) => {
    const raw = String(v);
    if (/^-?\d+(?:\.\d+)?$/.test(raw)) return raw;
    return `"${raw.replace(/"/g, '\"')}"`;
  }).join(',');
  const rows = await request(`/${encodeTableName(table)}?${safeColumn}=in.(${encodeURIComponent(inList)})`, {
    method: 'DELETE',
    headers: { Prefer: 'return=representation' },
  });
