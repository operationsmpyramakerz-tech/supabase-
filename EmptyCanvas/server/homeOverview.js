...
function rowDate(row = {}) { return text(row.createdAt ?? row.created_at ?? row.createdTime ?? row.notionCreatedTime ?? row.notion_created_time ?? row.date ?? row.requestDate ?? row.request_date); }
...
function stockTagName(item = {}) { return text(item?.tag?.name ?? item?.tag) || "Untagged"; }
